	...
	if(avatarp)
	{
		...
		if(distance<30)//if the object is close insert it in the attention model database
		{
			...
			if(found ==std::string::npos)
			{
				...
				if(objectp->isAvatar())// if the object is an avatar, add more body parts to enhance attention towards it
				{
					...
					ax=0, ay=0, az=0, aw=0;
					rot.getAngleAxis(&aw,&ax,&ay,&az);
					strcpy(tmpid,objectId); strcat(tmpid,"lefteye");
					updateNode(tmpid,position[0],position[2], position[1],ax,az,ay,aw);
					...
					ax=0, ay=0, az=0, aw=0;
					rot.getAngleAxis(&aw,&ax,&ay,&az);
					strcpy(tmpid,objectId); strcat(tmpid,"chest");
					updateNode(tmpid,position[0],position[2], position[1],ax,az,ay,aw);
					...
					ax=0, ay=0, az=0, aw=0;
					rot.getAngleAxis(&aw,&ax,&ay,&az);
					strcpy(tmpid,objectId); strcat(tmpid,"righteye");
					updateNode(tmpid,position[0],position[2], position[1],ax,az,ay,aw);
					...
					ax=0, ay=0, az=0, aw=0;
					rot.getAngleAxis(&aw,&ax,&ay,&az);
					strcpy(tmpid,objectId); strcat(tmpid,"torso");
					updateNode(tmpid,position[0],position[2], position[1],ax,az,ay,aw);
		...
		else//if the object is too far discard it from the attention model database
		{
			...
			if(objectp->isAvatar())// if the object is an avatar, delete body parts previously added
			{
				char tmpid[128];
				//remove Left Eyes
				strcpy(tmpid,objectId); strcat(tmpid,"lefteye"); removeNode(tmpid);
				//remove chest
				strcpy(tmpid,objectId); strcat(tmpid,"chest"); removeNode(tmpid);
				//remove Right Eye
				strcpy(tmpid,objectId); strcat(tmpid,"righteye"); removeNode(tmpid);
				//remove Torso
				strcpy(tmpid,objectId); strcat(tmpid,"torso"); removeNode(tmpid);
